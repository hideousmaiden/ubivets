    ...
    for rrr in range (2,1000):
        ...
        if sss == chat_id:
            ...
            results = service.spreadsheets().values().batchUpdate(spreadsheetId = fifile, body = {
                "valueInputOption": "USER_ENTERED", 
                "data": [
                    {"range": shit_iid+"!"+column_stat+str(nnn),
                     "majorDimension": "ROWS",     
                     "values": [[status],]}
            ]}).execute()
        else:
...
def add_friend(chat_id, text):
    ...
    for rrr in range (2,1000):
        ...
        if sss==chat_id:
            nnn=rrr
            rrr=rrr+1000
            ranges = [shit_iid+"!"+column_conn+str(nnn)] # 
            results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
            ...
            results = service.spreadsheets().values().batchUpdate(spreadsheetId = fifile, body = {
                "valueInputOption": "USER_ENTERED", 
                "data": [
                    {"range": shit_iid+"!"+column_conn+str(nnn),
                     "majorDimension": "ROWS",     
                     "values": [[befff, ';', text],]}
            ]}).execute()
        else:
...
def stats_reg(chat_id):
    ...
    kkk=0
    for rrr in range (2,1000):
        ranges = [shit_iid+"!"+column_stat+str(rrr)] # 
        results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
...
def part_nametaker(text, chat_id):
    ranges = [shit_iid+"!"+column_name+"2:"+column_name+"1000"] # 
    results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
        ...
        for rrr in range (2,1000):
            ...
            if sss==chat_id:
                ...
                results = service.spreadsheets().values().batchUpdate(spreadsheetId = fifile, body = {
                    "valueInputOption": "USER_ENTERED", 
                    "data": [
                        {"range": shit_iid+"!"+column_name+str(nnn),
                         "majorDimension": "ROWS",     
                         "values": [[text],]}
                ]}).execute()
            else:
...
def part_startgame(chat_id):
    ...
    for rrr in range (2,1000):
        ...
        if sss==chat_id:
            nnn=rrr
            rrr=rrr+1000
            ranges = [shit_iid+"!"+column_vict+str(nnn)] # 
            results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
...
def part_killed(chat_id):
    ...
    for rrr in range (2,1000):
        ...
        if sss==chat_id:
            nnn=rrr
            rrr=rrr+1000
            ranges = [shit_iid+"!"+column_name+str(nnn)] # 
            results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
    ...
    for hhh in range(2,1000):
        ranges = [shit_iid+"!"+column_vict+str(hhh)] # 
        results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
        ...
        if jjj==ubit:
            mmm=hhh
            hhh=hhh+1000
            ranges = [shit_iid+"!"+column_name+str(mmm)] # 
            results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
    ...
    for zzz in range (2,1000):
        ranges = [shit_iid+"!"+column_name+str(zzz)] # 
        results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
    ...
    results = service.spreadsheets().values().batchUpdate(spreadsheetId = fifile, body = {
        "valueInputOption": "USER_ENTERED", 
        "data": [
            {"range": shit_iid+"!"+column_vict+str(mmm),
             "majorDimension": "ROWS",     
             "values": [[new_pray],]}]
            }).execute()
    ...
    if text =='Статистика':
        ...
        for rrr in range (2,1000):
            ...
            if sss=='pl':#НУЖНЫ СТАТУС
                wwww=rrr
                ranges = [shit_iid+"!"+column_name+str(wwww)] # 
                results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
...
def main_body(m):
    ...
    for rrr in range(2,1000):
        ...
        if sss == user_id:
            nnn = rrr
            rrr = rrr + 1000
            ranges = [shit_iid+"!"+column_stat+str(nnn)] # 
            results = service.spreadsheets().values().batchGet(spreadsheetId = fifile, 
                                     ranges = ranges, 
                                     valueRenderOption = 'FORMATTED_VALUE',  
                                     dateTimeRenderOption = 'FORMATTED_STRING').execute()
